...
object EventTimeRestractSteamWithLongTS {
...
//      """select news_entry_id,ts,ts1 from user_behavior"""
    ...
    tEnv.toAppendStream[Row](tEnv.sqlQuery("""select news_entry_id,ts,ts1 from user_behavior""")).print()

        val countSql =
          """
            |SELECT news_entry_id,hour(tumble_start(ts1, interval '10' second)),count(*)
            |FROM user_behavior
            |GROUP BY news_entry_id,TUMBLE(ts1, INTERVAL '10' SECOND)
          """.stripMargin
    val table: Table = tEnv.sqlQuery(countSql)
    //table.printSchema()
    tEnv.toRetractStream[Row](table).print("aaa")

    //println(env.getStreamGraph.getJobGraph)
    env.execute()
  }
}
